...
with tab_manager:
    ...
    if st.session_state.logged_in:
        ...
        # LOAD DATA
        try:
            # ttl=5 prevents the Quota Error
        ...
        if not df.empty:
            ...
            if not filtered_df.empty:
                ...
                st.warning("⚠️ No data matches filter.")

            st.markdown("---")

            # 5. SMOOTH EDIT/DELETE SECTION (FIXED)
            with st.expander("🛠️ Wax ka bedel / Tirtir (Edit/Delete)", expanded=True):
                if not filtered_df.empty:
                    ...
                    # --- FORM START (Stops Loading on every click) ---
                    with st.form("batch_editor_form"):
                    ...
                    # Logic Handling after Form Submit
                    if save_btn:
                        try:
                    ...
                    if delete_btn:
                        if edited_df["Select"].any():
                            ...
                            st.warning("⚠️ Fadlan marka hore xulo safafka (Please select rows first).")

                    # CONFIRMATION BOX (Outside Form)
